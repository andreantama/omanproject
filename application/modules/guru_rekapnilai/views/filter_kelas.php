...
<div class="panel panel-default">
	...
	<h3>Rekap nilai <?= $detail_mapel->MAPEL ?></h3>
	<hr />
	<h4>Siswa kelas <?= $detail_kelas->NAMA_KELAS ?></h4>
<hr/>
	<div class="form-group">
		<a href="javascript:void(0);" class="btn btn-primary" onclick="window.open('<?= base_url('guru_rekapnilai/cetakRekapNilaiMapel/'.$this->jariprom_tools->base64_encode_fix($detail_kelas->ID_KELAS).'/'.$detail_semester.'/'.$id_mapel) ?>', '_blank', 'width=800,height=600,scrollbars=yes,status=yes,resizable=yes,screenx=0,screeny=0');">Cetak Nilai</a>
	</div>
		...
		<tbody>
			...
			<tr valign="middle" align="center">
				...
				<?php
					...
					$ul_ts = $this->guru_rekapnilai->getNilaiUts(array('NO_GURU' => $this->session->userdata('user_access_id'),'ID_TAHUN_PEL' => $detail_semester, 'NO_SISWA' => $rekap_nilai->NO_SISWA, 'ID_MAPEL' => $id_mapel));
					$nilai_uas = $this->guru_rekapnilai->tampilData('tbl_nilai_uas','*', array('ID_TAHUN_PEL' => $detail_semester, 'NO_SISWA' => $rekap_nilai->NO_SISWA, 'ID_MAPEL' => $id_mapel), TRUE);
					$uas = (isset($nilai_uas->UAS)) ? $nilai_uas->UAS : 0;
					$uu_2 = (isset($nilai_uas->UAS)) ? $nilai_uas->UAS : 0;
				
					$detail_nilai = $this->guru_rekapnilai->tampilData('tbl_nilai_mapel','*', array('NO_GURU' => $this->session->userdata('user_access_id'),'ID_TAHUN_PEL' => $detail_semester, 'NO_SISWA' => $rekap_nilai->NO_SISWA, 'ID_MAPEL' => $id_mapel), TRUE);
					if($detail_nilai){
