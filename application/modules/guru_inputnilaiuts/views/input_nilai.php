					...
					<div class="col-md-4">
						...
						<div class="form-group">
							...
							<select class="form-control" id="id_mapel" name="id_mapel">
								...
								<?php
									$get_rel_mapel = $this->guru_inputnilaiuts->tampilData('tbl_rel_kelasajar', '*', array('ID_KELAS' => $id_kelas, 'NO_GURU' => $this->session->userdata('user_access_id')));
									if($get_rel_mapel){
								...
								<?php foreach($list_mapel as $list_mapel): ?>
									<?php
										...
										$detail_mapel = $this->guru_inputnilaiuts->tampilData('tbl_mapel','*', array('ID_MAPEL' => $list_mapel->ID_MAPEL), TRUE);
									?>
...
<script>
	$('#id_mapel').change(function(){
		...
		$.ajax({
			method : "POST",
			url : '<?= base_url("guru_inputnilaiuts/filterNilai") ?>',
			data : 'id_mapel=' + $('#id_mapel').val() + '&id_siswa='  +  $('#id_siswa').val() + '&id_kelas='  +  $('#id_kelas').val(),
