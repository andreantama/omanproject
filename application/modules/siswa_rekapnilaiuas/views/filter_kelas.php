...
<hr/>
	<div class="form-group">
		<a href="javascript:void(0);" class="btn btn-primary" onclick="window.open('<?= base_url('siswa_rekapnilaiuas/cetakRekapNilaiUas/'.$this->jariprom_tools->base64_encode_fix($detail_kelas->ID_KELAS).'/'.$detail_semester) ?>', '_blank', 'width=800,height=600,scrollbars=yes,status=yes,resizable=yes,screenx=0,screeny=0');">Cetak Nilai</a>
	</div>
			...
			<tr valign="middle" align="center">
				...
				<td><?= ($rekap_nilai->JK == 1 ? "L" : "P") ?></td>
					...
					<?php
						...
						foreach($sql as $key){
							if($key->ID_MAPEL != 1){
								$detail_nilai = $this->siswa_rekapnilaiuas->tampilData('tbl_nilai_uas','*', array('NO_SISWA' => $rekap_nilai->NO_SISWA, 'ID_TAHUN_PEL' => $detail_semester, 'ID_MAPEL' => $key->ID_MAPEL), TRUE);
								if($detail_nilai){
