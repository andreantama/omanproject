...
class M_walikelas_inputperingkat extends CI_Model {
	...
	function existPeringkatKelas($data){

		$no_siswa = $data['NO_SISWA'];
		
		unset($data['NO_SISWA']);
		
		$result = $this->tampilData('tbl_nilai_peringkat','*', $data, TRUE);
		
		if(!$result)
			return false;
		
		if($result->PERINGKAT != $data['PERINGKAT'])
			return false;

		if($result->NO_SISWA == $no_siswa)
			return false;
				
		return true;
		
	}
	function getPeringkatKelas($data){
		
		$result = $this->tampilData('tbl_nilai_peringkat','*', $data, TRUE);
		
		if($result)
			return $result->PERINGKAT;
